# 测试 prompt · 2023 高教社杯 A 题 · 定日镜场的优化设计
...
你要完整作答 **2023 年高教社杯（全国大学生数学建模竞赛）A 题"定日镜场的优化设计"**，独立做出可提交的国赛风格方案与论文。
...
你的项目根目录是：`D:/PythonProject/数学建模skill/math-modeling-skill-suite/2023A/`。
...
- `research/research.md`——题目理解、资料检索、候选方法、用户（你 agent 自己）的判断与新想法
- `paper/writer.md`——论文大纲、占位符、待确认结论
- `code/code.md`——脚本、数据、参数、指标、可复现说明
- `2023A/review.md`（或按 skill 习惯放 `paper/review_report.md`）——独立审查报告
...
1. 起手：`cd 2023A && git init -b main`（如未初始化）
2. 在阶段 1 末尾做一次初始提交：`feat: init 2023A project skeleton & first-response`
...
> **重要：你每次回复尽量都跑一次 git 命令**——既留可复盘轨迹，也方便外部观察者按 commit 复现你的思考节奏。
...
2. 在 `research/research.md` 用 `templates/stage-files/research.md` 模板写调研第一稿：
   - 题目理解与难点
   - 检索方向（关键词 + 渠道：Kaggle / GitHub / CSDN / Google Scholar / CNKI / arXiv）
   - 已掌握的公式与待补足的公式（点名阴影遮挡模型、截断效率模型、镜场布局算法）
   - 候选方法（蒙特卡洛光线追迹、Campo / Radial Stagger / NSGA-II / 差分进化）与初步取舍理由
   - 至少 5 条要追问自己的"下一阶段门禁问题"
3. 检索动作至少做 3 类：
   - 算法类文献（含 2018 年张平等公开论文）
   - 历届 CUMCM 真题中类似题目（搜索"定日镜场"、"塔式光热"、"solar power tower"、"heliostat field optimization"）
   - 行业实践案例（青海德令哈、敦煌塔式电站公开报告）
4. 把每条检索的真实链接（必须能打开）写入 `research/research.md`，绝不编造来源。
5. **`research/research.md` 中必须明确写出你选择给本仓库压力测试用的附件获取策略**：
   - 优先：尝试下载官方公开附件 URL（请使用 `WebFetch` 或 `curl https://r.jina.ai/...` 试探）。如果网络不可达允许失败，不能凭幻觉写"已下载"。
   - 次优：明确写"使用文献 N 篇中给出的虚拟镜面位置 / 已合成数据集"，给出生成脚本。
   - **绝对禁止** 编造"我刚下载了附件"或伪造坐标。
6. 这一阶段结束时**立刻 git commit**：

   ```bash
   cd "D:/PythonProject/数学建模skill/math-modeling-skill-suite/2023A"
   git add -A
...
1. 在 `research/research.md` 末尾记录三个问题的**候选模型路线**及推荐路线，并写清楚权衡。
2. 在 `paper/writer.md` 中：
   - 列大纲（按 CUMCM 框架：摘要 / 问题重述 / 问题分析 / 模型假设 / 符号说明 / 数据处理 / 模型建立与求解 / 结果分析 / 灵敏度 / 评价与推广 / 参考文献 / 附录）
   - **预先放好** 占位图表（命名约定：`figures/q1_monthly_efficiency.png`、`figures/q1_annual_summary.png`、`figures/q2_layout.png`、`figures/q3_mixed_layout.png`、`figures/sensitivity.png`、`figures/method_flow.png` 等）
...
1. 论文草稿：`paper/main.tex` 写入完整章节正文（不允许代码未完成就写"结果"）。与代码同步推进，每填一张表就把文件名 + 数据契约同步到 `paper/writer.md`。
2. 代码：实现三个问题的求解器，至少包含：
   - `q1_baseline.py`：在给定 1745 面（或同等规模）6×6 镜面、4m 安装高度、固定塔中心时计算 60 个时点的光学效率与单位面积输出热功率，输出月表与年表（写 `figures/q1_*.png` 与 `outputs/result1.xlsx`）。
   - `q2_uniform_mirror.py`：在 60 MW 约束下等尺寸优化（推荐差分进化/NSGA-II 一类启发式），输出 `outputs/result2.xlsx`、`figures/q2_layout.png`。
   - `q3_mixed_mirror.py`：在 60 MW 约束下不同尺寸混合的优化，输出 `outputs/result3.xlsx`、`figures/q3_mixed_layout.png`。
   - `common/astro.py`：太阳高度角/方位角/DNI。**必须** 设随机种子。
   - `common/monte_carlo.py`：单面镜面反射光线追迹，用于阴影遮挡与截断效率。
3. **不许** 编造实验数值，论文里所有具体数字必须来自 code 真实输出（在论文里引用 `(详见 code/q1_baseline.py)`）。
4. 每完成一个 Python 文件即在 `code/code.md` 记录：脚本路径、输入、输出、参数、随机种子、指标、失败尝试。
5. 至少画一张建模流程图（建议 `figures/method_flow.png`），用 `templates/figures/flowchart-placeholder.md` 引导。
6. 阶段结束 commit。
...
4. `outputs/result_summary.xlsx`（重命名自三张表的副本之一）保存汇总。
...
5. 审查报告落盘在 `paper/review_report.md`（或 `2023A/review.md`）。
6. 阶段结束 commit。
...
1. `outputs/` 下复制最终交付物：
   ...
   - `outputs/result1.xlsx` / `result2.xlsx` / `result3.xlsx` / `result_summary.xlsx`
