...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{raw_vs_denoised.png}
\caption{原始 vs 去噪反射率光谱对比 (sym8 小波, level=6, 通用阈值)。}
\label{fig:raw-dn}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{sensitivity_N.png}
\caption{Q2 厚度对载流子浓度 $N$ 的扫描 ($N \in [10^{15}, 10^{20}]$~cm$^{-3}$, $\gamma=30$~cm$^{-1}$)。$d$ 在 $10^{17}$–$3\times 10^{19}$~cm$^{-3}$ 区间稳定于 7.4~µm, 仅在 $N \gtrsim 10^{19}$~cm$^{-3}$ 后漂移。}
\label{fig:sensitivity-N}
\end{figure}
...
\subsection{模型选择判据 (主推 Q2, 辅以 Q3)}

本文同时给出 Q2 (极值点反演) 与 Q3 (TMM-FP 全谱反演) 两组结果, 现明确其权重:
\begin{enumerate}
\item \textbf{Q2 极值法 —— 主推荐}: $d_\text{SiC} = 7.444 \pm 0.044~\mu$m (双角度加权)。其优点是对 $n(\tilde{\nu})$ 模型细节依赖弱, 仅作用于条纹间距尺度; 不受 Sellmeier 占位系数的系统性偏移。
\item \textbf{Q3 TMM-FP —— 辅参考}: $d_\text{SiC}^\text{TMM} \approx 6.57~\mu$m。该方法依赖 $n(\tilde{\nu})$ 全谱细节, 而本文采用的 Sellmeier 占位系数在 RMSE ~18\% 量级, 故给出 $d$ 的系统性偏低。
\end{enumerate}

\noindent \textbf{结论}: 主结果采用 Q2 = $7.44 \pm 0.04~\mu$m, TMM-FP 在 §9 局限段报告。
