# 测试 prompt · 2025 高教社杯 B 题 · 碳化硅外延层厚度的反演
...
你要完整作答 **2025 年高教社杯（全国大学生数学建模竞赛）B 题"碳化硅外延层厚度反演"**，独立做出可提交的国赛风格方案与论文。
...
你的项目根目录是：`D:/PythonProject/数学建模skill/math-modeling-skill-suite/2025B/`。
...
- `paper/review_report.md`（或 `2025B/review.md`）——独立审查报告
...
1. 起手：`cd 2025B && git init -b main`（如未初始化）
2. 在阶段 1 末尾做一次初始提交：`feat: init 2025B project skeleton`
...
2. 在 `research/research.md` 用 `templates/stage-files/research.md` 模板写调研第一稿：
   - 题目理解与难点（**重点**：光谱 → 厚度是反问题，需明确折射率色散、Drude 模型、热噪声、干涉级数识别）
   - 检索方向（关键词 + 渠道：Google Scholar "infrared interferometry SiC epitaxial thickness"、GitHub `CUMCM 2025 B`、arXiv "Sellmeier Drude SiC"、Web of Science）
   - 候选方法（极值点拟合法 / 全谱拟合法 / 传输矩阵法 TMM / 贝叶斯反演 / 神经网络 surrogate）
   - 已掌握模型 + 至少 5 条"下一阶段门禁问题"
3. 检索动作至少做 3 类：
   - 算法类（红外干涉法、SiC 折射率建模、TMM 计算）
   - 历届 CUMCM 真题相似题（干涉条纹测厚、半导体厚度反演）
   - 行业实践（KLA-Tencor / Filmetrics / SENTECH 商业仪器原理）
4. 把每条检索的真实链接（必须能打开）写入 `research/research.md`，绝不编造来源。
5. **`research/research.md` 中必须明确写出你选择给本仓库压力测试用的附件获取策略**：
   - 优先：尝试下载官方公开附件（`WebFetch` / `curl https://r.jina.ai/...` / `gh search`）。
   - 备选：基于 Skyler-Luo README 给出的"4 份附件（10°/15° × SiC/Si）波数约 1000~5000 cm⁻¹"格式**合成** 4 个 `附件N.csv`。在 README 注明合成方法。
   - **绝对禁止** 编造"已下载"。
6. 这一阶段结束时**立刻 git commit**：

   ```bash
   cd "D:/PythonProject/数学建模skill/math-modeling-skill-suite/2025B"
   git add -A
...
1. 在 `research/research.md` 末尾记录三个问题的**候选模型路线**及推荐路线，并写清楚权衡（重点：极值点法快但噪声敏感；全谱 + TMM 更鲁棒但慢）。
2. 在 `paper/writer.md` 中：
   - 列大纲（按 CUMCM 框架）
   - **预先放好** 占位图表（命名约定：`figures/raw_spectra.png`、`figures/denoised_spectra.png`、`figures/extrema_fit.png`、`figures/n_surface.png`、`figures/multi_beam_condition.png`、`figures/multi_angle_compare.png`、`figures/method_flow.png` 等）
...
2. 代码：实现三个问题的求解器，至少包含：
   - `common/sellmeier.py`：Sellmeier 折射率模型
   - `common/drude.py`：Drude 模型（载流子浓度依赖）
   - `common/optical_path.py`：光程差与干涉级数
   - `data_preprocess.py`：附件格式转换 + 小波去噪
   - `q1_model.py`：单次反射干涉数学模型推导 + 数值验证
   - `q2_extremum_fitter.py`：极值点拟合法求解 SiC 外延层厚度（附件 1、2）
   - `q3_full_spectrum.py`：全谱拟合 + TMM 多光束分析（附件 3、4 + 反向作用于附件 1、2）
   - `multi_angle_compare.py`：多角度交叉验证 + 不确定度估计
   - 输出 `figures/` 与 `outputs/result1.xlsx` / `result2.xlsx` / `result3.xlsx`
...
5. 审查报告落盘在 `paper/review_report.md`（或 `2025B/review.md`）。
6. 阶段结束 commit。
